...
export async function renderAccount({ root, api, esc, updateIdentity, updateCsrf, isActive, signOut }) {
  ...
  try {
    ...
    root.innerHTML = `<div class="account-workspace">
      <section class="account-section"><div><h2>Your account</h2><p class="helper-text">${account.authMethod === "unknown" ? "Signed-in account." : `Signed in with ${account.authMethod === "google" ? "Google" : "email and password"}.`}</p></div>
      ...
      <section class="account-section"><div><h2>Signed-in sessions</h2><p class="helper-text">${account.sessions.length} active ${account.sessions.length === 1 ? "session" : "sessions"}</p></div><div><ul class="account-sessions">${account.sessions.map((session) => `<li><div><strong>${session.current ? "This session" : "Other session"}</strong><span>${session.method === "google" ? "Google" : session.method === "password" ? "Email & password" : "Existing session"}</span><small>Signed in ${esc(sessionDate(session.createdAt))}</small></div>${session.current ? '<span class="status-badge is-active">Current</span>' : `<button class="secondary" type="button" data-revoke-session="${esc(session.id)}">Sign out</button>`}</li>`).join("")}</ul><button id="signout-others" type="button" class="secondary" ${account.sessions.length < 2 ? "disabled" : ""}>Sign out other sessions</button><p id="sessions-message" role="status"></p><button id="account-signout" type="button" class="secondary">Sign out of this account</button></div></section>
    </div>`;
