...
function storeView() {
  ...
  setPageHeader(
    ...
    `<a class="secondary button-link" href="/api/stores/${storeId}/storefront/preview" target="_blank" rel="noopener">Preview</a>${(data.storefrontPublication?.live || status === "published") ? ` <a class="primary button-link" href="/s/${encodeURIComponent(data.store.slug)}" target="_blank" rel="noopener">Open Live Store</a>` : ""}`,
  );
  ...
  const storeSections = [
    ...
    ["connections", "Products & pages", "Choose each product’s page"],
  ];
  ...
  const connectionRows = (data.products || []).map((product) => {
    ...
    return `<article class="store-connection-row"><div><strong>${esc(product.name)}</strong><small>${connected?.status === "published" ? "Published in this store" : "Product storefront needs publishing"}</small></div><label class="field">Connected product page<select data-store-page-for="${product.id}" aria-label="Page for ${esc(product.name)}"><option value="">Choose a published page</option>${pages.map((page) => `<option value="${page.id}" ${page.id === connected?.pageId ? "selected" : ""}>${esc(page.title)}</option>`).join("")}</select></label><div class="store-connection-actions"><button type="button" class="primary" data-store-connect="${product.id}" ${pages.length ? "" : "disabled"}>Save connection</button><button type="button" class="secondary" data-store-open-editor="${product.id}">Edit page</button><button type="button" class="secondary" data-store-product-data="${product.id}">Product data</button>${connected?.status === "published" ? `<a class="secondary button-link" target="_blank" rel="noopener" href="/s/${encodeURIComponent(data.store.slug)}/products/${encodeURIComponent(product.slug)}">View product</a>` : ""}</div><small>Product → selected page → checkout → order</small></article>`;
  }).join("");
  ...
  $("#product-editor").onsubmit = async (event) => {
    ...
    try {
      ...
      if (values.status === "active")
        open(`/s/${data.store.slug}/${page.liveSlug || page.slug}`, "_blank");
    } catch (error) {
      toast(error.message);
    }
  };
    ...
    data.pages.length
      ? `<table><thead><tr><th>PAGE</th><th>PROJECT</th><th>PRODUCT</th><th>METHOD</th><th>REVIEWS</th><th>URGENCY</th><th>STATUS</th><th>ACTIONS</th></tr></thead><tbody>${data.pages
          .map((p) => {
            ...
            return `<tr><td><strong>${esc(p.title)}</strong><br><small>/${esc(p.slug)}</small></td><td>${esc(p.projectName || "Legacy page")}</td><td>${esc(p.productName)}</td><td><span class="pill">${esc(p.creationMethod || "blank")}</span></td><td>${esc(options.proof)}</td><td>${esc(options.urgency)}</td><td><span class="pill">${esc(p.status)}</span></td><td><button class="secondary edit-page" data-id="${p.id}">Edit</button> <a class="link" target="_blank" href="/api/stores/${storeId}/pages/${p.id}/preview">Preview ↗</a> ${p.status === "draft" ? `<button class="secondary publish" data-id="${p.id}">Publish</button>` : `<a class="link" target="_blank" href="/s/${esc(data.store.slug)}/${esc(p.slug)}">Open live ↗</a>`}</td></tr>`;
          })
          .join("")}</tbody></table>`
...
function orderColumnsMenu(preferences){return `<details class="row-menu columns-menu"><summary class="secondary">Columns</summary><div class="order-columns-list">${preferences.columnOrder.map((key)=>`<div draggable="true" data-order-column="${key}"><span aria-hidden="true">☰</span><span>${esc(orderColumnLabels[key])}</span><button type="button" data-toggle-order-column="${key}" aria-label="${preferences.visibleColumns.includes(key)?"Hide":"Show"} ${esc(orderColumnLabels[key])}">${preferences.visibleColumns.includes(key)?"◉":"○"}</button></div>`).join("")}</div></details>`;}
...
function domainsView() {
  ...
  const showWizard = (domain = null, step = 1) => {
    ...
    const render = (current, activeStep) => {
      ...
      if (activeStep === 1) {
        modalContent.innerHTML = `<div class="domain-wizard"><span class="eyebrow">Advanced setup</span><h2>Connect Custom Domain</h2>${steps}<div class="domain-dns-help"><strong>Before continuing, confirm your DNS provider supports CNAME and custom TXT records.</strong><span>Some free DNS panels only support limited TXT records. For easiest testing, use the store link instead: <code>${esc(defaultStoreUrl)}</code>.</span></div><p>Enter the exact hostname customers should use, for example <strong>shop.example.com</strong>.</p><label class="field">Domain Name<input name="domainName" placeholder="shop.example.com" autocomplete="url" required></label><p class="helper-text">Enter only the hostname. Do not repeat the provider suffix if your DNS panel appends it automatically.</p><div class="wizard-actions"><button class="secondary" id="domain-cancel" type="button">Cancel</button><button class="primary" type="submit">Continue</button></div></div>`;
...
function liveVisitorsView(visitorData = data.liveVisitors) {
  ...
  content.innerHTML = `<div class="live-visitor-tabs" role="tablist" aria-label="Live visitor activity">${filters
    ...
    visible.length
      ...
      : `<div class="live-empty"><div class="live-empty-visual" aria-hidden="true"><svg viewBox="0 0 48 48" fill="none"><rect x="7" y="10" width="34" height="28" rx="5"/><path d="M7 18h34M12 14h1m4 0h1M13 30h6l4-8 5 12 3-4h5"/></svg></div><h3>${emptyTitle}</h3><p>${emptyCopy}</p>${!trackingEnabled ? '<button type="button" class="secondary" id="live-privacy-settings">Review privacy settings</button>' : liveVisitorFilter === 'all' ? `<a class="secondary button-link" href="${data.storefrontPublication?.live ? '/s/' + encodeURIComponent(data.store.slug) : '/api/stores/' + storeId + '/storefront/preview'}" target="_blank" rel="noopener">${data.storefrontPublication?.live ? 'Open your store' : 'Preview your store'} <span aria-hidden="true">↗</span></a>` : '<button type="button" class="secondary" id="live-show-all">Show all visitors</button>'}</div>`
...
const simplifiedProductPages = (() => {
  ...
  function pagesView() {
    ...
    const pageActions = (page) =>
        `<details class="row-menu"><summary aria-label="Actions for ${esc(page.title)}">⋯</summary><div><button class="edit-page" data-id="${page.id}" type="button">Edit</button><a target="_blank" href="/api/stores/${storeId}/pages/${page.id}/preview">Preview</a>${page.status === "published" ? `<a target="_blank" href="/s/${esc(data.store.slug)}/${esc(page.liveSlug || page.slug)}">View Live</a>` : ""}<button type="button" data-page-rename="${page.id}">Rename</button><button type="button" data-page-duplicate="${page.id}">Duplicate</button>${page.status === "published" ? `<button type="button" data-page-unpublish="${page.id}">Unpublish</button>` : ""}<button class="danger-text" type="button" data-page-delete="${page.id}">Delete</button></div></details>`,
      pageRows = productPages
        .map(
          (page) =>
            `<tr><td><strong>${esc(page.title)}</strong></td><td>${esc(page.productName)}</td><td><small>/s/${esc(data.store.slug)}/${esc(page.slug)}</small></td><td>${esc(methodLabels[page.creationMethod] || "Legacy")}</td><td><span class="status-badge ${page.status === "published" ? "is-active" : "is-draft"}">${page.status === "published" ? "Published" : "Draft"}</span></td><td class="row-menu-cell">${pageActions(page)}</td></tr>`,
        )
        .join(""),
      pageCards = productPages
        .map(
          (page) =>
            `<article class="product-page-mobile-card"><header><div><strong>${esc(page.title)}</strong><small>${esc(page.productName)}</small></div>${pageActions(page)}</header><code>/s/${esc(data.store.slug)}/${esc(page.slug)}</code><dl><div><dt>Method</dt><dd>${esc(methodLabels[page.creationMethod] || "Legacy")}</dd></div><div><dt>Status</dt><dd><span class="status-badge ${page.status === "published" ? "is-active" : "is-draft"}">${page.status === "published" ? "Published" : "Draft"}</span></dd></div></dl></article>`,
        )
        .join("");
    setPageHeader(
      selectedProduct
