...
const db=new DatabaseSync(resolve(process.argv[2] || 'data/commera2.sqlite'),{readOnly:true});
...
const giftCredit=columns.has('gift_card_applied_paise')?'COALESCE(gift_card_applied_paise,0)':'0';
try {
  console.log(JSON.stringify({
    ...
    invalidOrderTotals:count(`SELECT COUNT(*) count FROM orders WHERE total_paise<>MAX(0,subtotal_paise-discount_paise+shipping_paise-${giftCredit})`),
