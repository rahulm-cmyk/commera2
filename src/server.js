...
import { readJsonBody as body } from './request-input.js';
import { renderCodBuilder, renderCodLauncher } from './cod-builder.js';
import { listShippingRates, saveShippingRate, saveShippingWeights } from './shipping-rules.js';
...
const secureResponse = (req, res) => {
  ...
  if (/\/(?:checkout|thank-you|upsell)\//.test(String(req.url || ''))) {
  ...
  Object.fromEntries(
    String(req.headers.cookie || "")
      ...
      .map((item) => {
        ...
        try { return [item.slice(0, index), decodeURIComponent(item.slice(index + 1))]; }
        catch { return [item.slice(0, index), '']; }
      }),
  );
...
function publicPage({
  ...
  const privacyBanner = privacy.cookieBannerEnabled
    ? `<aside id="cookie-banner" class="cookie-banner"><p>${htmlEscape(privacy.bannerMessage)}</p>${privacyLink ? `<a href="${htmlEscape(privacyLink)}">Privacy Policy</a>` : ""}<button id="accept-cookies" type="button">${htmlEscape(privacy.acceptButtonText)}</button><button id="reject-cookies" type="button">${htmlEscape(privacy.rejectButtonText)}</button></aside><script>document.querySelector('#accept-cookies').onclick=()=>{storageSet('local','commera2_tracking_consent_${Number(page.storeId)}','accepted');document.querySelector('#cookie-banner').remove();window.commera2LoadPixels?.()};document.querySelector('#reject-cookies').onclick=()=>{storageSet('local','commera2_tracking_consent_${Number(page.storeId)}','rejected');document.querySelector('#cookie-banner').remove()};try{if(localStorage.getItem('commera2_tracking_consent_${Number(page.storeId)}'))document.querySelector('#cookie-banner')?.remove()}catch{}</script>`
    : "";
  const field = (
    key,
    name,
  ...
  const media = storefrontProduct?.media || {},
    ...
    heroBundleOptions = bundles.length
      ...
      : "",
    purchaseAction = product.stock < 1
      ? '<button type="button" class="hero-cta" disabled>Sold out</button><small class="buy-note">This product is currently unavailable.</small>'
      : `${heroBundleOptions}<label class="hero-quantity">Quantity<input id="hero-quantity" type="number" min="1" max="${product.stock}" value="1"></label><a href="${checkoutHref}" class="hero-cta" ${directCheckout ? 'data-direct-checkout="true"' : ""}>${ctaText}</a><small class="buy-note">Cash on Delivery available · Secure order confirmation</small>`,
    heroProductInfo = `<div class="product-purchase"><span class="eyebrow">${htmlEscape(hero.eyebrow || "SHOP THE PRODUCT")}</span><p class="product-name">${htmlEscape(product.name)}</p><h1>${htmlEscape(hero.headline || product.name)}</h1>${ratingMarkup}<div class="product-price"><strong id="hero-price">${money(product.pricePaise)}</strong>${product.comparePricePaise ? `<del>${money(product.comparePricePaise)}</del>` : ""}${discountPercent ? `<b>Save ${discountPercent}%</b>` : ""}</div>${hero.subheadline || product.description ? `<p class="product-short-description">${htmlEscape(hero.subheadline || product.description)}</p>` : ""}${product.stock < 1 ? '' : urgencyHtml}${purchaseAction}<small id="checkout-launch-status" class="checkout-launch-status" role="status"></small></div>`;
  ...
  ${renderCodLauncher(cod, page, directCheckout)}
  ${stickyMobile && product.stock > 0 ? `<div class="sticky-mobile-buy"><strong>${money(product.pricePaise)}</strong><a href="${checkoutHref}" class="hero-cta" ${directCheckout ? 'data-direct-checkout="true"' : ""}>${ctaText}</a></div>` : ""}
...
function checkoutPage({
  ...
  const template =
    ...
    selectedPrice = bundle ? bundle.pricePaise : product.pricePaise * selectedQuantity,
  ...
  <script>const SESSION_ID=${scriptJson(checkout.id)},STORE_ID=${Number(page.storeId)},CURRENCY=${scriptJson(currency)},SAVE_INCOMPLETE=${Boolean(cod.saveIncompleteCheckout && checkoutSettings.captureAbandonedCheckout && privacy.allowAbandonedCheckoutData)},HARD_DISABLED=${product.stock < 1 || !cod.buttonEnabled},INITIAL_PINCODE=${scriptJson(checkout.pincodeValidated ? checkout.pincode : "")},CHECKOUT_TRACKING=${scriptJson({productId:product.id,pageId:page.id,quantity:selectedQuantity,bundleId:bundle?.id||null,checkoutValue:Number(checkout.totalPaise)/100})};</script>
...
export function createApp({
  ...
  const completedCheckoutUrl = (storeId, sessionId, resolvedDomain) => {
    const store = service.getStore(storeId);
    if (!service.hasCompletedCheckout(store.slug, sessionId)) return null;
    const prefix = resolvedDomain?.storeId === storeId ? '' : `/s/${encodeURIComponent(store.slug)}`;
    return `${prefix}/thank-you/${encodeURIComponent(sessionId)}`;
  };
    ...
    try {
      ...
      if (path === '/api/integrations/google-sheets/callback' && req.method === 'GET') {
        ...
        try {
          ...
          if (!session || session.sessionId !== pending.sessionId) throw Error('Sign in to Commera2 again, then connect Google from the same browser.');
          if (pending.state !== url.searchParams.get('state')) throw Error('This connection request is no longer current. Click Connect Google account again.');
          if (url.searchParams.get('error') === 'access_denied') throw Error('Google access was cancelled. Allow Google Sheets access to connect.');
          if (!url.searchParams.get('code') || url.searchParams.has('error')) throw Error('Google did not authorize the connection. Please try again.');
          ...
          const detail = /^(Sign in to Commera2|This connection request|Google access was cancelled|Google did not authorize|Google sign-in session|Allow Google Sheets access|Google Sheets connection needs)/.test(messageText)
            ? messageText.slice(0,180) : 'Google could not complete the connection. Check the configured callback URL and try connecting again.';
      ...
      const sheetMatch = path.match(/^\/api\/stores\/(\d+)\/utm-sheet(?:\/(connect|inspect|create|add-column|pause|sync))?$/);
      if (sheetMatch) {
        ...
        if (req.method === 'POST' && action === 'create') return json(res, 201, await utmSheets.createSheet(id, await body(req)));
      ...
      match = path.match(/^\/api\/stores\/(\d+)\/shipping-rates(?:\/(\d+|weights))?$/);
      if (match) {
        const storeId=Number(match[1]);service.getStore(storeId);
        if(req.method==='GET'&&!match[2])return json(res,200,listShippingRates(db,storeId));
        if(req.method==='PUT'&&match[2]==='weights')return json(res,200,saveShippingWeights(db,storeId,(await body(req)).weights));
        if(req.method==='POST'&&!match[2])return json(res,201,saveShippingRate(db,storeId,null,await body(req)));
        if(req.method==='PATCH'&&/^\d+$/.test(match[2]||''))return json(res,200,saveShippingRate(db,storeId,Number(match[2]),await body(req)));
      }
      ...
      if (match && req.method === "PATCH") {
        ...
        const receipt = completedCheckoutUrl(storeId, match[1], resolvedDomain);
        if (receipt) return json(res, 409, { error: 'This order is already confirmed', completed: true, nextUrl: receipt });
        const
      ...
      if (match && req.method === "POST") {
        const input = await body(req),
          storeId = Number(input.storeId);
        const receipt = completedCheckoutUrl(storeId, match[1], resolvedDomain);
        if (receipt) return json(res, 200, { completed: true, thankYouUrl: receipt, nextUrl: receipt });
        const
        ...
        const concurrentReceipt = completedCheckoutUrl(storeId, match[1], resolvedDomain);
        if (concurrentReceipt) return json(res, 200, { completed: true, thankYouUrl: concurrentReceipt, nextUrl: concurrentReceipt });
      ...
      if (match && req.method === "POST") {
        ...
        await pixels
          ...
          .catch(() => {});
        const nextUrl = result.nextOffer
          ? `${resolvedDomain?.storeSlug === result.interaction.storeSlug ? '' : `/s/${encodeURIComponent(result.interaction.storeSlug)}`}/upsell/${encodeURIComponent(result.interaction.checkoutSessionId)}?offer=${result.nextOffer.id}&token=${encodeURIComponent(result.nextOffer.token)}`
          : thankYouUrl;
        return json(res, 200, { ...result, nextUrl });
      }
      ...
      if (req.method === "GET") {
        ...
        const files = {
          ...
          "/store-package.js": "store-package.js",
