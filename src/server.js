...
function checkoutPage({
  ...
  function validateControl(control){if(!control||control.disabled||control.readOnly||['hidden','radio','button','submit'].includes(control.type))return true;if(control.name==='alternatePhone'){const primary=form.elements.phone?.value.trim(),alternate=control.value.trim();control.setCustomValidity(alternate&&alternate===primary?'Use a different number for the alternate phone.':'')}const target=errorTarget(control),valid=control.validity.valid;control.classList.toggle('is-invalid',!valid);control.setAttribute('aria-invalid',String(!valid));if(target)target.textContent=valid?'':fieldMessage(control);return valid}
  ...
  function handleStatus(){const message=status.textContent.trim();if(!message){status.hidden=true;return}if(message==='Details saved'){couponPending=false;status.textContent='';status.hidden=true;return}if((couponPending||/coupon/i.test(message))&&couponStatus){couponStatus.textContent=message;couponStatus.dataset.state='error';document.querySelector('#checkout-coupon-code')?.setAttribute('aria-invalid','true');couponPending=false;status.textContent='';status.hidden=true}else{status.hidden=false;status.dataset.state='error';routeError(message)}checkoutSubmitting=false;if(!hardDisabled)submitButton.disabled=false}
  ...
  form.addEventListener('invalid',event=>{event.preventDefault();validateControl(event.target)},true);form.addEventListener('input',event=>{const control=event.target;if(control.setCustomValidity){const previousError=control.validationMessage;control.setCustomValidity('');if(previousError&&status.textContent===previousError){status.textContent='';status.hidden=true}}if(control.name==='phone')validateControl(form.elements.alternatePhone);validateControl(control);if(control===pincode){if(pincodeStatus){pincodeStatus.textContent='';delete pincodeStatus.dataset.state}if(!hardDisabled)submitButton.disabled=false}},true);
